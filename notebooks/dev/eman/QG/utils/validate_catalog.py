...
def all_params(directory: str = None):
    all_params = {}

    if directory is None:
        directory = Path("./catalog.yaml")
    else:
        directory = Path(directory).joinpath("./catalog.yaml")

    cat = intake.open_catalog(directory)
    for item in cat:
        ...
        if "region" not in params.keys():
            ...
            cat_kwargs = [
                {"region": i[0], "datatype": i[1], "season": i[2]} for i in cat_kwargs
            ]

            if "grid" in params.keys():
                more_kwargs = [p for p in product(params["region"], ["grid"], grids)]
                more_kwargs = [
                    {"region": i[0], "datatype": i[1], "grid": i[2]}
                    for i in more_kwargs
                ]
