...
def steepest_descent(
    ...
    # initialize state
    state = SDResults(
        u=u_init, residual=residual, b=b,
        iteration=0,
...
def conjugate_gradient(
    ...
    def body_fn(state):
        ...
        # compute stepsize
        alpha = jnp.sum(state.residual * state.residual) / \
            jnp.sum(state.direction * A_direction)
        ...
        beta = jnp.sum(residual * residual) / \
            jnp.sum(state.residual * state.residual)
