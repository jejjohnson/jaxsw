...
def laplacian_2d(array: Array, dx: Array, dy: Array) -> Array:
    lap_kernel_x = jnp.array([[1.0, -2.0, 1.0]])
...
def advection_term_upwind(q, psi, dx, dy, way: int = 1):
    ...
    # forward methods
    dq_dx_f = fdx.difference(
        q, step_size=dx, axis=0, derivative=1, accuracy=1, method="forward"
    )
    dq_dy_f = fdx.difference(
        q, step_size=dy, axis=1, derivative=1, accuracy=1, method="forward"
    )
    # backward methods
    dq_dx_b = fdx.difference(
        q, step_size=dx, axis=0, derivative=1, accuracy=1, method="backward"
    )
    dq_dy_b = fdx.difference(
        q, step_size=dy, axis=1, derivative=1, accuracy=1, method="backward"
    )
