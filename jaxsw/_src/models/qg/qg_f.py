...
class Qgm:
        ...
        **kwargs
        ...
        if self.mdt is not None:
            if mdu is None or mdv is None:
                ...
                self.ubar = mdu
                self.vbar = mdv
                self.qbar = self.huv2pv(
                    mdt, mdu, mdv, c=np.nanmean(self.c) * np.ones_like(self.dx)
                )
        ...
        x, y = np.meshgrid(
            np.arange(1, nx - 1, dtype="float64"), np.arange(1, ny - 1, dtype="float64")
        )
        laplace_dst = (
            2 * (np.cos(np.pi / (nx - 1) * x) - 1) / np.mean(dx) ** 2
            + 2 * (np.cos(np.pi / (ny - 1) * y) - 1) / np.mean(dy) ** 2
        )
        self.helmoltz_dst = (
            self.g / self.f0.mean() * laplace_dst
            - self.g * self.f0.mean() / self.c.mean() ** 2
        )
    ...
    def h2pv(self, h, hbc, c=None):
        ...
        q = q.at[1:-1, 1:-1].set(
            ...
            / self.f0[1:-1, 1:-1]
            * (
                (h[2:, 1:-1] + h[:-2, 1:-1] - 2 * h[1:-1, 1:-1])
                / self.dy[1:-1, 1:-1] ** 2
                + (h[1:-1, 2:] + h[1:-1, :-2] - 2 * h[1:-1, 1:-1])
                / self.dx[1:-1, 1:-1] ** 2
            )
            - self.g * self.f0[1:-1, 1:-1] / (c[1:-1, 1:-1] ** 2) * h[1:-1, 1:-1]
        )
    ...
    def qrhs(self, u, v, q, uls=None, vls=None, qls=None, way=1):
        ...
        if not self.diff:
            ...
            if uls is not None:
                ...
                rq = rq.at[2:-2, 2:-2].set(
                    rq[2:-2, 2:-2]
                    + self._rq_jit(uplusls, vplusls, uminusls, vminusls, q)
                )
                # rq[2:-2,2:-2] = rq[2:-2,2:-2] + self._rq(
